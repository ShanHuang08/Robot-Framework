...
class ExternalApiUser(HttpUser):
    ...
    def get_pokemon_apis(self, amount:int):
        """Utilize `get()` tp test pokemon APIs"""
        self.url_connection(self.host + self.api_v2)
  
        for i in range(0, amount):
            api = self.api_v2 if i == 0 else f"{self.api_v2}{i}"
            res = self.client.get(api)
            if res.status_code != 200:
                error(f"Status code is not 200 but {res.status_code}")
    
    def test_pokemon_apis(self, amount:int):
        """Utilize `request()` tp test pokemon APIs"""
        self.url_connection(self.host + self.api_v2)
